...
resource "null_resource" "exp" {
  ...
  provisioner "local-exec" {
    command = "cd /home/ec2-user/Ansible/ALL_ENV_Ansible ; ansible-playbook -i inv-dev -e ansible_user=ec2-user -e ansible_password=DevOps321 -e COMPONENT=${var.name} -e ENV=dev expense.yml"
  }
}
